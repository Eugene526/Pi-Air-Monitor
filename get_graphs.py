...
def get_graphs(db_wrapper, is_main_graphs=False, sensor='SDS011'):
    graphs = []

    if sensor == 'SDS011':
        sources = ['PM25', 'PM10']
    else:
        sources = SOURCES  # 原本 VOC 的欄位
    ...
    for source in sources:
        # 只處理有資料的欄位，避免 KeyError
        x = [utc_to_local(d['at']) for d in data if source in d]
        y = [float(d[source]) for d in data if source in d]
        ...
        # title
        title = format_title(source, y[0]) if y else 'no data'

        graph_div = html.Div([
            html.Div([html.H3(title)], className='Title'),
            dcc_graph,
